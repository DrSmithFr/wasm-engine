...
func (k *KeyboardOnly) Init(window *element.Window) {
	k.bindEvents(window)
}
...
func (k *KeyboardOnly) bindEvents(window *element.Window) {
	...
	window.Document().Js().Call("addEventListener", "keydown", keydownEventHandler)
	window.Document().Js().Call("addEventListener", "keyup", keyupEventHandler)
}
